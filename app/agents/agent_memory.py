...
_MEMORY_SYSTEM_PROMPT = (
    ...
    "CRITICAL — MEMORY IS A RECORD, NOT A RULE (never auto-act from it):\n"
    "- Store WHAT happened, never a standing instruction or a learned habit. A one-off "
    "action the user requested is NOT a pattern.\n"
    "- NEVER write memory that says or implies 'the user always/usually wants X' or 'auto-do "
    "X next time'. Do NOT generalise a past reply into a rule.\n"
    "- Past events must NEVER cause the agent to auto-act on a future similar event. If a "
    "similar email/message arrives later, the agent still just does its role (e.g. notify) "
    "and waits — it must NOT reply on its own because 'Maharshi replied like this before'.\n"
    "- Do not infer preferences/patterns from individual events.\n"
    "Rules:\n"
