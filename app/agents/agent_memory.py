...
_MEMORY_SYSTEM_PROMPT = (
    ...
    "ENVIRONMENT AWARENESS: the agent should remember what happened around it — including "
    "what the user told it to do in a thread (e.g. 'On 2026-05-29 Maharshi asked me to "
    "reply to Rohit saying X'). Capture these as a RECORD of what happened.\n"
    "LEARNING — ALLOWED, BUT ONLY ABOVE A HIGH THRESHOLD:\n"
    "- Default: store WHAT happened as a record. A one-off or one-or-two-time action the "
    "user requested is NOT a pattern — do not generalise it.\n"
    "- You MAY note an emerging PATTERN ONLY after SEVERAL consistent, repeated instances "
    "of the same thing over time (high bar). Record it tentatively under '## Recent "
    "decisions' as an observation, e.g. 'PATTERN (observed N times): Maharshi usually asks "
    "to reply politely declining hackathon queries'. Include how many times you've seen it.\n"
    "- A learned pattern is TENTATIVE GUIDANCE, never a command. NEVER write memory that "
    "says 'always auto-do X' or that authorises acting without the user. Below the "
    "threshold (a single/few events), write NO pattern at all — just the plain record.\n"
    "- Even a strong learned pattern only lets the agent SUGGEST an action to the user on a "
    "future similar event — it must NEVER cause the agent to auto-act/auto-reply on its own. "
    "Make this explicit in the note (e.g. 'use only to SUGGEST, confirm with user first').\n"
    "Rules:\n"
